...
write_stress_file <- function(filename=NULL,stresses){
  if(is.null(filename))
    filename='mystress.csv'
  write.table(stresses,file=filename,sep=",",row.names = F, col.names = F,append = TRUE)
}
...
filename = paste0("geom_",toString(i),".csv")
stressfile = paste0("wss_",toString(i),".csv")
write_mesh_file(filename=filename,nodes=filemat$V,triangles=filemat$T)
write_stress_file(filename=stressfile,stresses=filemat$wss)
}
